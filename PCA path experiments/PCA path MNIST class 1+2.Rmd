...
```{r}
pca1 = prcomp(Z1, rank. = 3)

df = data.frame(PC1 = pca1$x[,1],
                PC2 = pca1$x[,2],
                PC3 = pca1$x[,3])

ggplot(df, aes(x=PC1, y=PC2, color=factor(cluster1))) +
  geom_point()

ggplot(df, aes(x=PC1, y=PC3, color=factor(cluster1))) +
  geom_point()
```

```{r}
tree1 = get_mst(dist(Z1, method="manhattan"))

print_mst(pca1$x[,c(1,2)], tree1, cluster1)

# run_app(Z1, pca1$x[,c(1,2)], cluster1, id1)
```