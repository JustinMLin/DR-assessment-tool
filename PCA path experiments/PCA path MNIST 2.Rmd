...
```{r}
sp = get_shortest_path(tree, which(id == 20887), which(id == 6368))

path_ids = as.numeric(sp$vpath)
path_pts = Z_pca[path_ids,]

keep = kmeans_cluster == 7 & X[,1] < 4
...
```{r}
pca_full = prcomp(Z_new, rank.=3)
sum(pca_full$sdev[1:2]^2)/sum(pca_full$sdev^2)
